...
/*
* @return The number of items we failed to add to inventory.
* 
* The logic here is:
...
int AInventory::addItemToInventory(const FItemData& itemToAdd, const int quantity, const float durability)
{
	...
	UE_LOG(LogTemp, Warning, TEXT("AInventory::addItemToInventory(): We still have %d items to add."), totalNumberToAdd);
	updateWeight();
...
*/
int AInventory::addItemToSlot(const FItemData& itemData, const int quantity, const float durability, const int index)
{
	// 1. If the index is out of bounds, try adding the item to the inventory normally.
	if (index < 0 || index >= inventory.Num())
	{
		UE_LOG(LogTemp, Warning, TEXT("AInventory::addItemToSlot(): Cannot add item to slot %d because it is out of bounds. Will try to add to other slots."), index);
...
int AInventory::addItemToEmptySlot(const FItemData& itemData, const int quantity, const int index, const float durability)
{
	int totalNumberToAdd = quantity;
...
*/
bool AInventory::removeItemFromInventory(const FName itemId, const int quantity)
{
	...
	for (int index = inventory.Num() - 1; index >= 0; index--)
	{
		...
		if (inventory[index].itemId == itemId)
		{
			...
			int removedFromThisSlot = FMath::Min(inventory[index].itemQuantity, totalNumberToRemove);
	...
	if (totalNumberToRemove > 0)
	{
		UE_LOG(LogTemp, Warning, TEXT("AInventory::removeItemFromInventory(): Could not remove %d items because there were not enough in inventory."), totalNumberToRemove);
		return false;
	}
...
void AInventory::resizeInventoryArray()
{
	...
	if (inventory.Num() < totalNumberOfSlots)
	{
		...
		UE_LOG(LogTemp, Warning, TEXT("AInventory::resizeInventory(): Inventory resized to %d slots. Added empty slots."), totalNumberOfSlots);
		return;
	}
	...
	if (inventory.Num() > totalNumberOfSlots)
	{
		UE_LOG(LogTemp, Warning, TEXT("AInventory::resizeInventory(): Inventory resized to %d slots. Dropping extra items."), totalNumberOfSlots);
		while (inventory.Num() > totalNumberOfSlots)
		{
